...
impl HttpProxy {
    fn new(args: BootArgs) -> Self {
        Self {
            ...
            session_manager: args.session_manager.unwrap(),
        }
    }
    ...
    let session_id = req.uri().path().split(":").collect::<Vec<&str>>().last().unwrap_or(&"").to_string();

    let ipv6_addr = self.session_manager.get_or_create(&session_id, || {
        match self.ipv6_subnet {
            Some(v6) => Self::get_rand_ipv6(v6.first_address().into(), v6.network_length()),
    ...
    if Method::CONNECT == req.method() {
        ...
        if ipv6_addr != Ipv6Addr::UNSPECIFIED {
            connector.set_local_address(Some(ipv6_addr.into()));
        } else if let Some(ip) = self.fallback {
